...
with open("C:\\Users\\Raynah_Cheng1\\OneDrive - Baylor University\\Spatial_Biology\\All_Data\\6K_RNA_Data\\BRM961B_022_6KPanel_exprMat_file.csv", "r") as file:
    ...
    with open('6K_RNA_HER2.csv', mode = 'w') as file1, \
    open('6K_RNA_ERPR.csv', mode = 'w') as file2, \
    open('6K_RNA_TNBC.csv', mode = 'w') as file3, \
    open('6K_RNA_Primary.csv', mode = 'w') as file4, \
    open('6K_RNA_Met.csv', mode = 'w') as file5:
    ...
    for core_sample, group in grouped:
        ...
        if core_name in groupings['HER2']:
            her2_cores.append(core_name)
            group.to_csv('6K_RNA_HER2.csv', mode = "a", index = False, header = False)
        
        if core_name in groupings['ER/PR']:
            
            erpr_cores.append(core_name)
            group.to_csv('6K_RNA_ERPR.csv', mode = "a", index = False, header = False)

        if core_name in groupings['TNBC']:
            
            tnbc_cores.append(core_name)
            group.to_csv('6K_RNA_TNBC.csv', mode = "a", index = False, header = False)

        if core_name in groupings['Primary']:
            
            primary_cores.append(core_name)
            group.to_csv('6K_RNA_Primary.csv', mode = "a", index = False, header = False)

        if core_name in groupings['Met']:
            
            met_cores.append(core_name)
            group.to_csv('6K_RNA_Met.csv', mode = "a", index = False, header = False)
