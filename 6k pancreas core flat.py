...
with open("C:\\Users\\Raynah_Cheng1\\OneDrive - Baylor University\\Spatial_Biology\\All_Data\\6K_RNA_Data\\PA722_151_6KPanel_exprMat_file.csv", "r") as file:
    ...
    with open('6K RNA Normal.csv', mode='w') as file1, \
        open('6K RNA NAT.csv', mode='w') as file2, \
        open('6K RNA Grade 2.csv', mode='w') as file3, \
        open('6K RNA Grade 3.csv', mode='w') as file4, \
        open('6K RNA Basal.csv', mode='w') as file5, \
        open('6K RNA Classical.csv', mode='w') as file6, \
        open('6K RNA Over or Equal to 60.csv', mode='w') as file7, \
        open('6K RNA Under 60.csv', mode='w') as file8:
    ...
    for core_sample, group in grouped:
        ...
        if core_name in groupings['Normal']:
            group.to_csv('6K RNA Normal.csv', mode = "a", index = False, header = False)

        if core_name in groupings['NAT']:
            group.to_csv('6K RNA NAT.csv', mode = "a", index = False, header = False)

        if core_name in groupings['Grade 2']:
            group.to_csv('6K RNA Grade 2.csv', mode = "a", index = False, header = False)

        if core_name in groupings['Grade 3']:
            group.to_csv('6K RNA Grade 3.csv', mode = "a", index = False, header = False)
